...
class InkHavenInitialAppRoute extends StatefulWidget {
  ...
  @override
  State<InkHavenInitialAppRoute> createState() =>
      _InkHavenInitialAppRouteState();
}

class _InkHavenInitialAppRouteState extends State<InkHavenInitialAppRoute>
    with SingleTickerProviderStateMixin {
  final Tween<double> _titleOpacyTween = Tween(begin: 0, end: 1);
  final Tween<Offset> _titlePositionTween =
      Tween<Offset>(begin: const Offset(0, 0.5), end: Offset.zero);
  final Tween<Offset> _loginButtonPositionTween =
      Tween<Offset>(begin: const Offset(1, 0), end: Offset.zero);
  final Tween<double> _loginButtonOpacyTween = Tween<double>(begin: 0, end: 1);
  final Tween<Offset> _signUpButtonPositionTween =
      Tween<Offset>(begin: const Offset(1, 0), end: Offset.zero);
  ...
  void initState() {
    ...
    _titleOpacyAnimation = _titleOpacyTween.animate(CurvedAnimation(
        parent: _animationController,
        curve: const Interval(0, 0.6, curve: Curves.easeIn)));
    _titlePositionAnimation = _titlePositionTween.animate(CurvedAnimation(
        parent: _animationController,
        curve: const Interval(0, 0.62, curve: Curves.easeIn)));
    _loginButtonOpacyAnimation = _loginButtonOpacyTween.animate(CurvedAnimation(
        parent: _animationController,
        curve: const Interval(0.4, 0.65, curve: Curves.easeIn)));
    _loginButtonPositionAnimation = _loginButtonPositionTween.animate(
        CurvedAnimation(
            parent: _animationController,
            curve: const Interval(0.3, 0.65, curve: Curves.easeIn)));
    _signUpButtonOpacyAnimation = _signUpButtonOpacyTween.animate(
        CurvedAnimation(
            parent: _animationController,
            curve: const Interval(0.5, 0.7, curve: Curves.easeIn)));
    _signUpButtonPositionAnimation = _signUpButtonPositionTween.animate(
        CurvedAnimation(
            parent: _animationController,
